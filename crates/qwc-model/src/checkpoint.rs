...
impl Stats {
    ...
    pub fn engine_bytes(&self) -> u64 {
        self.packed_bytes + self.scale_bytes + self.plain_bytes
            + self.embed_bytes + self.lm_head_bytes + self.mtp_bytes
    }
}
...
impl Checkpoint {
    ...
    pub fn quant_linear(&self, prefix: &str, out: usize, inp: usize) -> Result<QuantLinear<'_>, String> {
        let packed_name = format!("{prefix}.weight_packed");
        let scale_name = format!("{prefix}.weight_scale");

        let packed_info = self.info(&packed_name).ok_or(format!("нет {packed_name}"))?;
        ...
        if packed_info.dtype != StDtype::U8 {
            return Err(format!("{packed_name}: тип {}, ожидался u8", packed_info.dtype.name()));
        }
        ...
        if scale_info.dtype != StDtype::F8E4m3 {
            return Err(format!("{scale_name}: тип {}, ожидался f8_e4m3", scale_info.dtype.name()));
        }
    ...
    pub fn validate(&self) -> (Vec<String>, Stats) {
        ...
        let check_plain = |name: &str, want: &[usize], st: &mut Stats, problems: &mut Vec<String>| {
        ...
        for name in [names::FINAL_NORM] {
            check_plain(name, &[HIDDEN_SIZE], &mut st, &mut problems);
            seen.push(name.into());
        }
        ...
        for (name, field) in [
            ...
            match self.info(name) {
                ...
                Some(t) => {
                    ...
                    if field == 0 { st.embed_bytes += b } else { st.lm_head_bytes += b }
                    st.tensors += 1;
                }
            }
            seen.push(name.into());
