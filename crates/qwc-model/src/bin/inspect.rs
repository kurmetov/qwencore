...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  не нужно (vision)     {:>7.2} GB  ({} тензоров)", g(st.unused_bytes), st.unused_tensors);
    println!("  итого на диске        {:>7.2} GB",
    ...
    let ours = st.packed_bytes + st.scale_bytes + st.plain_bytes + st.mtp_bytes + head_fp8 + embed_fp8;

    println!("\nЧто грузим мы (text-only, lm_head и embed в fp8)");
    println!("  vLLM грузит           {:>7.2} GB", g(st.engine_bytes() + st.unused_bytes));
    println!("  мы грузим             {:>7.2} GB", g(ours));
    println!("  фора                  {:>7.2} GB", g(st.engine_bytes() + st.unused_bytes - ours));

    let cfg = CacheConfig::default();
    let b = rtx5090(ours, 2 * GB);
    println!("\nПод кэш остаётся        {:>7.2} GB", g(b.cache_available()));
