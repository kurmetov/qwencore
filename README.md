...
| измерено, обычный decode | 75.8 tok/s | **72% потолка** |
| измерено, спекуляция k=3 + шортлист 32K | 159.1 tok/s | 3.36 токена на проход, знаменатель другой, см. ниже |
...
Обе строки «измерено» — медианы трёх прогонов одной сессии 2026-09-24 на
частично занятой карте, без доверительных интервалов; методика —
[`speculation-corpus-2026-09-17.md`](bench/results/speculation-corpus-2026-09-17.md),
замер — [`packed-attention-2026-09-24.md`](bench/results/packed-attention-2026-09-24.md),
раздел «Продолжение». Прежние 79.2 и 138.5 сняты 09-17: обычный decode той
же сборки до правок дня в этой сессии даёт 76.4, так что разница с 79.2 — карта,
а не код; спекуляция выросла за счёт упакованного внимания и прогрева головы.
...
токен, перплексии и task-eval нет. Дифф-eval гоняет бинарник `eval`, который
работает с исполнителем напрямую, без планировщика, кэша префиксов и
спекуляции; к `serve` с настройками по умолчанию его выводы не относятся, см.
[ниже](#спекуляция-и-кэш-префиксов). Спекуляции при batch > 1 нет вовсе: сервер
включает её, только когда генерирует один запрос. Кэш префиксов замерен одной
сессией, без интервалов. Без замеров FP8-внимание на префилле и CUTLASS W4A4
на малом M.
...
- `POST /v1/completions` (без chat template, без streaming; для сверки с `eval`)
...
### Спекуляция и кэш префиксов

Оба включены по умолчанию.

| флаг | по умолчанию | что делает |
|---|---|---|
| `--speculative K` | 3 | MTP-голова предлагает K черновиков, модель проверяет их одним проходом. Работает, только когда генерирует один запрос; голова занимает ещё 0.85 GB |
| `--shortlist N` | 32768 | черновик считает логиты по N частым токенам плюс токенам контекста, а не по всему словарю |
| `--mtp-prime N` | 2048 | голова прогревается по последним N токенам промпта, если в сервере одна последовательность |
| `--prefix-cache N` | 8 | снимков состояния DeltaNet на границе истории (~81 MB каждый); следующий ход диалога не пересчитывает историю |

Оба меняют greedy-выход на почти равных токенах. Проверка черновиков идёт
префилльным путём, и её логиты сходятся с decode не бит в бит. Кэш режет
каждый промпт на последнем `<|im_start|>`, даже когда попадания нет, и
короткий хвост после разреза считается рекуррентным сканом, а не чанковым.
Численные утверждения дифф-eval относятся к пути `eval` при batch 1. С
`--prefix-cache 0 --speculative 0` сервер при одном запросе совпадает с
`eval` бит в бит, если у обоих одинаковый `--context`: от него зависит
разбиение внимания на партиции. Спекуляция вносит шум уровня decode, токены на
корпусах дифф-eval совпадают все. Кэш префиксов на длинных промптах порог
дифф-eval не проходит (MAE 0.41), причина не разобрана. Для сравнения: одна
перестановка порядка fp32-сложения во внимании префилла даёт против прежней
сборки 0.11–0.13, так что порог 0.25 для правок префилла строг. При нескольких одновременных
запросах совпадения нет в любом случае: на batch 8 дифф-eval строгий порог
не проходит. Разбор —
[`experiment-report-2026-09-24.md`](bench/results/experiment-report-2026-09-24.md).

Проверка черновиков идёт упакованным ядром внимания (все шесть голов
GQA-группы — строки одного тайла), и на длинном контексте спекуляция
выигрывает: на 30k токенов 165 tok/s против 73.5 у обычного decode, на 8k —
187 против 76.5 (один запрос). Промпты тут — исходники этого репозитория, и
продолжение копирует код из контекста, так что приём черновиков высокий; на
другом тексте выигрыш будет меньше. Разбор —
[`packed-attention-2026-09-24.md`](bench/results/packed-attention-2026-09-24.md).

`POST /v1/completions` принимает промпт текстом или id токенов и отдаёт
`logprobs` (до 20); `ignore_eos` и `return_tokens_as_token_ids` — как у vLLM.
Им пользуется `bench/diff_eval.py --engine openai --openai-token-ids`.
