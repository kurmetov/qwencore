...
`eval` бит в бит, если у обоих одинаковый `--context`: от него зависит выбор
ядра внимания. Спекуляция вносит шум уровня decode, токены на корпусах
дифф-eval совпадают все. Кэш префиксов на длинных промптах порог дифф-eval не
проходит (MAE 0.41), причина не разобрана. При нескольких одновременных
...
На длинном контексте спекуляция сейчас проигрывает: на 30k токенов 33.5
tok/s против 61.2 у обычного decode. Проверка черновиков идёт префилльным
ядром внимания, у которого для нескольких строк нет разбиения по контексту.
Для агентов на длинном контексте пока лучше `--speculative 0`.
